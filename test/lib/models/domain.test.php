...
class DomainTest extends PHPUnit_Framework_TestCase {
    ...
    public function testFindOne() {
        foreach (self::$_data as $obj) {
            $dbObj = Domain::findOne(['id' => $obj->id]);
    ...
    public function testFind() {
        ...
        $dbObjects = Domain::findMulti(['name' => $regex]);
    ...
    /**
     * Tests adding multiple domains
     */
    public function testAddMulti() {
        $domain = "phpunit.com";
        $domains = [$domain, "www.$domain", $domain];

        Domain::addMulti($domains);
        Domain::addMulti($domains);

        $count = Domain::count(['name' => ['$in' => $domains]]);
        $this->assertEquals($count, count(array_unique($domains)), "Inserted multiple domains");

        Domain::deleteMulti(['name' => ['$in' => $domains]]);
    }
